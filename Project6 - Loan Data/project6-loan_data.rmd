...
loan$LoanOriginationQuarterSorted <- factor(loan$LoanOriginationQuarter, levels = c("Q1 2006", "Q2 2006", "Q3 2006", "Q4 2006", "Q1 2007", "Q2 2007", "Q3 2007", "Q4 2007", "Q1 2008", "Q2 2008", "Q3 2008", "Q4 2008", "Q1 2009", "Q2 2009", "Q3 2009", "Q4 2009", "Q1 2010", "Q2 2010", "Q3 2010", "Q4 2010", "Q1 2011", "Q2 2011", "Q3 2011", "Q4 2011", "Q1 2012", "Q2 2012", "Q3 2012", "Q4 2012", "Q1 2013", "Q2 2013", "Q3 2013", "Q4 2013", "Q1 2014"), ordered = TRUE)
...
ggplot(data= loan, aes(x = LoanOriginationQuarterSorted, fill = factor(loan$ListingCategory))) +
  geom_bar() +
  scale_x_discrete(drop = FALSE) +
...
loan$LoanStatus <- factor(loan$LoanStatus, levels = c("Cancelled", "Completed", "FinalPaymentInProgress", "Current", "Past Due (1-15 days)", "Past Due (16-30 days)", "Past Due (31-60 days)", "Past Due (61-90 days)", "Past Due (91-120 days)", "Past Due (>120 days)",  "Defaulted", "Chargedoff"), ordered = TRUE)
...
hash <- list(
  defaulted = c("Chargedoff","Defaulted","Cancelled", "Past Due (>120 days)", "Past Due (16-30 days)", "Past Due (31-60 days)", "Past Due (61-90 days)", "Past Due (91-120 days)"),
...
loan$Occupation <- factor(loan$Occupation, levels = loan$Occupation %>% fct_infreq() %>% levels(), ordered = TRUE)
...
ggplot(data = loan, aes(x = reorder(Occupation, StatedMonthlyIncome, median), y = StatedMonthlyIncome)) +
...
ggplot(data = subset(loan, EmploymentStatus %in% c("Full-time", "Part-time")), aes(x = reorder(Occupation, StatedMonthlyIncome, median), y = StatedMonthlyIncome, color = EmploymentStatus)) +
...
ggplot(data = loan, aes(x = EmploymentStatusDuration, y = StatedMonthlyIncome, color = Occupation)) +
...
ggplot(data = loan, aes(y= LoanOriginalAmount, x=StatedMonthlyIncome, color = IncomeVerifiable)) +
...
ggplot(data = subset(loan, loan$StatedMonthlyIncome > 1), aes(y= LoanOriginalAmount, x=StatedMonthlyIncome)) +
...
ggplot(data = loan, aes(x= LoanOriginalAmount, y=MonthlyLoanPayment, color = factor(Term))) +
  geom_point() +
  geom_smooth() +
...
loan$ProsperRating..Alpha. <- factor(loan$ProsperRating..Alpha. , levels =c("AA", "A", "B", "C", "D", "E", "HR")  , ordered = TRUE)

ggplot(data = subset(loan, !is.na(ProsperRating..numeric.) & MonthlyLoanPayment >1), aes(x= LoanOriginalAmount, y=MonthlyLoanPayment, color = ProsperRating..Alpha.)) +
  geom_point() +
...
ggplot(data= loan.by_rating_status, aes(x=ProsperRating..Alpha., y=mean_income, color= LoanStatusCompressed)) +
  geom_point() +
  xlab("Prosper Rating") +
...
ggplot(data = subset(loan, !is.na(ProsperRating..Alpha.)), aes(x = DebtToIncomeRatio, y = LoanOriginalAmount, color = ProsperRating..Alpha.)) +
...
Here it is interesting, that the Prosper Rating worked quite well, as from AA, A and B rated Borrowers only one single loan defaulted to this point. But apart from that, it is quite difficult to make out any difference an these plot. Thus, forecasting a defaulting loan is a quite hard task.


```{r echo=FALSE, Multivariate_Plots6}
# Filter only completed and defaulted loans, current loans are not of interest here
loan_2status <- subset(loan, LoanStatusCompressed %in% c("completed", "defaulted"))
...
ggplot(data = loan.by_Status_Category.long, aes(x = Category, y = Status, fill = Freq)) +
  geom_tile() +
...
loan.by_Status_Occupation <- prop.table(table(loan_2status$LoanStatusCompressed, loan_2status$Occupation), 2)
...
ggplot(data = loan.by_Status_Occupation.long, aes(x = Occupation, y = Status, fill = Freq)) +
  geom_tile() +
  scale_fill_gradientn(colours = terrain.colors(10)) +
  theme(axis.text.x=element_text(angle = 90, hjust = 1, vjust = 0.5)) 
```
...
loan.by_Occupation_Rating <- prop.table(table(loan$Occupation, loan$ProsperRating..Alpha.), 1)
...
ggplot(data = loan.by_Occupation_Rating.long, aes(x = Occupation, y = ProsperRating, fill = Freq)) +
  geom_tile() +
  scale_fill_gradientn(colours = terrain.colors(10)) +
  theme(axis.text.x=element_text(angle = 90, hjust = 1, vjust=0.5)) +
...
#head(sort(table(loan$MemberKey), decreasing = TRUE), 9)

# filter loan data set using these members keys to get only their "story"
## there must be a "nicer" way to convert the output of the command above to a vector, so these two lines can be one and I can avoid hard coding
top9TotalLoans <- subset(loan, loan$MemberKey %in% c("63CA34120866140639431C9", "16083364744933457E57FB9", "3A2F3380477699707C81385", "4D9C3403302047712AD0CDD", "739C338135235294782AE75", "7E1733653050264822FAA3D", "C45F3365236450678BA344C", "C70934206057523078260C7", "01C433656567683311715EE"))
top9TotalLoans$LoanOriginationQuarterSorted <- factor(top9TotalLoans$LoanOriginationQuarterSorted, levels = c("Q1 2006", "Q2 2006", "Q3 2006", "Q4 2006", "Q1 2007", "Q2 2007", "Q3 2007", "Q4 2007", "Q1 2008", "Q2 2008", "Q3 2008", "Q4 2008", "Q1 2009", "Q2 2009", "Q3 2009", "Q4 2009", "Q1 2010", "Q2 2010", "Q3 2010", "Q4 2010", "Q1 2011", "Q2 2011", "Q3 2011", "Q4 2011", "Q1 2012", "Q2 2012", "Q3 2012", "Q4 2012", "Q1 2013", "Q2 2013", "Q3 2013", "Q4 2013", "Q1 2014"), ordered = TRUE)
...
ggplot(aes(x = LoanOriginationQuarterSorted, y = StatedMonthlyIncome, label = Occupation),
       data = top9TotalLoans) +
  facet_wrap( ~ MemberKey, ncol = 3) +
  geom_point(aes(size = LoanOriginalAmount, colour = ListingCategory), pch =1) +
  geom_text(aes(ifelse(TotalProsperLoans == 8, Occupation, "")), hjust = "left", vjust = "top", alpha = 0.4, check_overlap = TRUE) +
  theme(axis.text.x=element_text(angle = 90, hjust = 0, vjust = 0.5)) +
  guides(fill=guide_legend(nrow=3)) 
```

So there is a computer programmer with varying income, who keeps on loaning for debt consolidation which sound like he is in a vicious circle.There is a Administrative Assistant about whom we do not have a lot information: debt consolidation, other and not available are the loan categories. A borrower of unknown profession who also needed loans for debt consolidation but also business use. A construction worker with no income and finally quite high income who needed loans for business, which seemed to pay off in the end. A Teacher who took loans for various reasons loke student use, debot consolidation and home improvement. A Nurse who improved her home quite a lot and needed loans for debt consolidation and apart from that gives us few information (Not Available) about the other loans. A person of unknown profession taking loans for medical reasons, for home improvement and other and a professional with a quite high income who solely takes loans for improving the house.

first is a programmer who gets loans around 5000 dollars solely for debt consolidation (and not available information). We can see an Administrative Assistant
